...
// ============================================================================
// Order-first money path — the contract the LIVE commerce-web /checkouts/:id
// page uses (invoice load → fees/discount quote → order create → PowerTranz
// 3DS). This is the discount-preserving, 3DS-hardened path; prefer it over the
// session path (which cannot apply discounts until commerce-api ships the fix).
// All money amounts are in MAJOR currency units. In Inkress the Product is the
// buyable unit — a line's `id` is a product id (there is no separate variant id
// today), and per-line variations ride in `properties`.
// ============================================================================
